...
// Extracted from the original natal-report-only implementation so any page
// with a NatalChart-shaped result (solar return, progressed, composite,
// Davison -- previously all showed a bare chart wheel with zero explanatory
// text) can get the same placement-by-placement reading instead of
// duplicating this logic per page.
export default function NatalPlacementList({ chart }: { chart: NatalChart }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // knowledge_base has a public read policy (no per-user scoping, see
      // migration 002) -- no session/token needed here.
